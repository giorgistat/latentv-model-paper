################################################################################
# Age-Stratified Analysis of AMA1 Antibody Data
#
# This script fits the latent Beta model to simulated AMA1 data
...
#
# This exploratory analysis (Section 5.1 of manuscript) guides the
...
cat("Starting AMA1 age-stratified analysis...\n\n")
...
cat("Loading simulated AMA1 data...\n")

# Load simulated data
data_ama <- readRDS("simulated_ama1_data.rds")

y_log <- data_ama$ama_log
age <- data_ama$age
...
write.csv(alpha_beta_df, "results/ama1_alpha_beta_by_age_group.csv", row.names = FALSE)
...
# Save plots
ggsave(
  "results/ama1_age_stratified_analysis.pdf", 
  fig_combined, 
  width = 14, 
  height = 6, 
  units = "in"
...
ggsave(
  "results/ama1_age_stratified_analysis.png", 
  fig_combined, 
  width = 14, 
  height = 6, 
  units = "in",
...
cat("  - Saved to: results/ama1_age_stratified_analysis.pdf\n")
cat("  - Saved to: results/ama1_age_stratified_analysis.png\n")
...
cat("\n", rep("=", 70), "\n", sep = "")
cat("AMA1 AGE-STRATIFIED ANALYSIS COMPLETE\n")
cat(rep("=", 70), "\n", sep = "")
...
cat(sprintf("  - Beta increases from %.3f to %.3f\n", 
            min(alpha_beta_df$beta), max(alpha_beta_df$beta)))
cat(sprintf("  - Mean T peaks at %.3f in age group %s\n",
            max(alpha_beta_df$mu), 
            alpha_beta_df$bracket[which.max(alpha_beta_df$mu)]))

cat("\nOutputs generated:\n")
cat("  - results/ama1_alpha_beta_by_age_group.csv\n")
cat("  - results/ama1_age_stratified_analysis.pdf\n")
cat("  - results/ama1_age_stratified_analysis.png\n\n")